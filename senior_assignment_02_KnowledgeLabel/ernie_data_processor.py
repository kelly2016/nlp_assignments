# -*- coding: utf-8 -*-
# @Time    : 2020-03-10 11:36
# @Author  : Kelly
# @Email   : 289786098@qq.com
# @File    : ernie_data_processor.py
# @Description:
...
all_labels = ['__label__高中_生物_生物技术实践', '__label__高中_历史_近代史', '__label__高中_政治_科学思维常识', '__label__高中_历史_古代史', '__label__高中_生物_生物科学与社会', '__label__高中_生物_稳态与环境', '__label__高中_生物_现代生物技术专题', '__label__高中_政治_经济学常识', '__label__高中_地理_人口与城市', '__label__高中_地理_区域可持续发展', '__label__高中_生物_分子与细胞', '__label__高中_地理_生产活动与地域联系', '__label__高中_地理_地球与地图', '__label__高中_政治_科学社会主义常识', '__label__高中_地理_宇宙中的地球', '__label__高中_政治_公民道德与伦理常识', '__label__高中_生物_遗传与进化', '__label__高中_政治_时事政治', '__label__高中_历史_现代史', '__label__高中_政治_生活中的法律常识']
...
def getMlb(dir):
    ...
    newDf = pd.DataFrame()
    newDf['labels'] = df['multiLabels'].apply(lambda x: x.split())
    mlb = MultiLabelBinarizer()
    y = mlb.fit_transform(newDf['labels'])
    y = [' '.join([str(j) for j in i]) for i in y.tolist()]
    newDf['labels'] = y
    newDf['content'] = df['item'].apply(lambda x: x.replace(' ',''))
    return mlb,newDf

def multiProcess(dir):
    mlb,df= getMlb(dir)
    classes_df = pd.DataFrame(mlb.classes_)
    df = shuffle(df)
    # Split dataset
    dir = dir + 'chnsenticorp' + os.sep+'multiCopus'+ os.sep
    df_train = df[:int(len(df) * 0.8)]
    df_train.to_csv(os.path.join(dir, 'train.tsv'), index=None, header=True, sep='\t')
    df_valid = df[int(len(df) * 0.8):int(len(df) * 0.9)]
    df_valid.to_csv(os.path.join(dir, 'dev.tsv'), index=None, header=True, sep='\t')
    df_test = df[int(len(df) * 0.9):]
    df_test.to_csv(os.path.join(dir, 'test.tsv'), index=None, header=True, sep='\t')
...
def getlb(dir):
    ...
    lb = LabelBinarizer()
    lb.fit(all_labels)#newDf['label']
    y = lb.transform(newDf['label'])
    y = np.argmax(y, axis=1)  # 返回沿轴axis最大值的索引。
    #y = [' '.join([str(j) for j in i]) for i in y.tolist()]
    newDf['label'] = y
    newDf['text_a'] = df['item'].apply(lambda x: x.replace(' ',''))
    return lb,newDf
...
def process(dir):
    ...
    lb, newDf = getlb(dir)
    newDf = shuffle(newDf)
    dir = dir + 'chnsenticorp' + os.sep+'singlecorpus'+ os.sep
    df_train = newDf[:int(len(newDf) * 0.8)]
    df_train.to_csv(os.path.join(dir, 'train.tsv'), index=None, header=True, sep='\t')

    df_valid = newDf[int(len(newDf) * 0.8):int(len(newDf) * 0.9)]
    df_valid.to_csv(os.path.join(dir, 'dev.tsv'), index=None, header=True, sep='\t')

    df_test = newDf[int(len(newDf) * 0.9):]
    df_test.to_csv(os.path.join(dir, 'test.tsv'), index=None, header=True, sep='\t')
