class HmmCut:
    def __init__(self,trans_path,emit_path,start_path):
    ...
    def viterbi(self, obs, states, start_p, trans_p, emit_p):
        # 维特比算法（一种递归算法）
        # 算法的局限在于训练语料要足够大
        # 需要给每个词一个发射概率,get(obs[0], 0)的用法是如果dict中不存在这个key,则返回0值
        V = [{}]
        path = {}
        for y in states:
        ...
        for t in range(1, len(obs)):
            ...
            newpath = {}
            for y in states:
                state_path = ([V[t - 1][y0] * trans_p[y0].get(y, 0) * emit_p[y].get(obs[t], 0), y0]
...
cuter = HmmCut(trans_path=trans_path,emit_path=emit_path,start_path=start_path)
seglist = cuter.cut(sent)
print(seglist)
print("End to cut words ")
